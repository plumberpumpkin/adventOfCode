...
fn day_two_part_one(input:[&str; 1000]){
    //initialize output variable
    let mut total_paper_amount:i32 = 0; 
    //Get Item from input
    for present in input{
        //split string in individual components and convert them into u16 type
        //refactor idea -> let dimensions: Vec<i32> = present.split('x').map(|x| x.parse::<i32>().unwrap()).collect();
        let dimensions_str: Vec<&str> = present.split('x').collect();
        let mut dimensions: Vec<i32> = Vec::new();
        for string in dimensions_str{
            dimensions.push(string.parse().unwrap());
        }
        //dimensions are L; W; H; 
        //calculate each side type
        
        let side_one: i32 = dimensions[0] * dimensions[1];
        let side_two: i32 = dimensions[1] * dimensions[2];  
        let side_three: i32 =dimensions[2] * dimensions[0];
        
        //get smallest side size
        let sides = vec![side_one, side_two, side_three];
        let smallest_side = sides.iter().min().unwrap();
    
        //calculate total area needed with formula 2 * (L*W + W*H + H*L) + (smallest side size)
        let present_paper_size = 2*(side_one + side_two + side_three) + smallest_side;
        total_paper_amount += present_paper_size;     
    }
    println!("The total amount needed is {}", total_paper_amount);
...
fn day_two_part_two(input:[&str; 1000]){
    //initialize output variable
    ...
    for present in input{
        ...
        dimensions.sort();
        println!("1: {}, 2: {}, 3: {}", dimensions[0], dimensions[1], dimensions[2]);
        let present: i32 = (2*dimensions[0]) + (2*dimensions[1]);
        println!("Ribbon needed for present: {}", present);
        let bow: i32 = dimensions[0] * dimensions[1] * dimensions[2];
        println!("Ribbon needed for bow: {}", bow);
        let present_amount = present + bow;
        println!("Total Ribbon needed for present: {}", present_amount);

        total_ribbon += present_amount;
        
    }

    println!("Total ribbon amount is {}", total_ribbon);

}

fn day_three_part_one(input: &str){

    //initialize position at X=0 & Y=0
    let mut position = (0, 0);
    //initialize result vector that stores every visited position
    let mut visited_houses: Vec<(i32, i32)> = Vec::new();

    //check each movement and log position in position vector
    for step in input.chars(){
        ...
        position.0 = next_X;
        position.1 = next_Y;
        //println!("Position X: {}, Position Y: {}", position.0, position.1);

        visited_houses.push(position);

    }

    //sort vector and remove duplicate positions
    visited_houses.sort();
    println!("Total visits: {}", visited_houses.len());
    
    visited_houses.dedup();
    println!("Individual houses reached: {}", visited_houses.len())
